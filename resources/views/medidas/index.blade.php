...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="datatable-medidas" class="table table-striped">
                    <thead>
                        <tr>

                            <th class="centrar">#</th>
                            <th class="centrar">Nombre</th>
                            <th class="centrar">Nombre corto</th>
                    ...
                    <tbody>
                        @foreach ($medidas as $medida)
                            <tr>
                                ...
                                {{--  <td>{{ \Carbon\Carbon::parse($medida->fecha)->format('d-m-Y') }}</td>  --}}
                                <td>
    ...
    <!--- Modal crear------>
    <div class="modal fade" id="CreateModal" tabindex="-1" role="dialog" aria-labelledby="miModalLabel"
        aria-hidden="true">
    ...
    <div class="modal fade" id="EditModal" tabindex="-1" role="dialog" aria-labelledby="miModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header text-center">
                    <h5 class="modal-title" id="miModalLabel">Editar Nueva Medida</h5>
                </div>

                <div class="modal-body">
                    <!-- Contenido del modal -->
                    <input type="hidden" value="{{ url('/') }}" id="url">
                    <form id="formedit-medida" action="{{ route('medidas.update', ['idmedida' => $medida->idmedida]) }}" method="POST">
                        <meta name="csrf-token" content="{{ csrf_token() }}">
                        @csrf
                        <div class="mb-3">
                            <label for="nombre_medida" class="form-label">Nombre</label>
                            <input type="text" class="form-control" id="nombre_medida_edit" name="nombre"
                                value="{{ $medida->nombre_medida }}">
                        </div>
                        <div class="mb-3">
                            <label for="nombre_corto" class="form-label">Nombre Corto</label>
                            <input type="text" class="form-control" id="nombrecorto_edit" name="nombrecorto"
                                placeholder="Ejemplo: PZ" value="{{ $medida->nombre_corto }}">
                        </div>
                        <div class="modal-footer">
                            <button id="cerrar" type="button" class="btn btn-secondary"
                                data-dismiss="modal">Cerrar</button>
...
@section('js')
    ...
    {{--  <script  src="{{ asset('js/activarformulario.js') }}"></script>  --}}
    <script src="{{asset('js/medidas.js') }}"></script>
    <script src="{{asset('js/editmedidas.js')}}"></script>


    <script type="text/javascript" charset="utf-8" async defer>
        $(document).ready(function() {
            var dt_medidas_table = $('#datatable-medidas')
            var dt = dt_medidas_table.DataTable({
                language: {
